      ...
      <b-col id="quartileCapture" cols="4" v-if="sortOrder === 'sorted'">
        <div class="table-container">
          <table :class="{ 'fade-in': sortOrder === 'sorted', 'fade-out': sortOrder === 'raw' }"class="table table-bordered  quartile-table-container" id='quartileTable'>
            <thead>
              <tr>
                <th style="width: 60%;" class="table-secondary">Agent</th>
...
const quartileDataArray = computed(() => {
  ...
  const array = Object.entries(quartileData.value).map(([agent, quartile]) => ({ agent, quartile }));
...
function addLinesBetweenQuartiles() {
  ...
  const agents = Object.keys(quartileData.value);

  // Iterate over the agents to find transitions between quartiles
  for (let i = 1; i < agents.length; i++) {
    const currentAgent = quartileData.value[agents[i]];
    const previousAgent = quartileData.value[agents[i - 1]];
...
function addQuartileLabels() {
  ...
  agents.forEach(agent => {
    const quartile = quartileData.value[agent].quartile;
  ...
  try {
    if (format === 'pdf') {
      ...
      if (showAdditionalTable.value) {
        const columns = ["Agent", "Quartile"]; // Define your columns

        // Extract data from quartileDataArray
        const rows = quartileDataArray.value.map(q => [q.agent, q.quartile.quartile]);
