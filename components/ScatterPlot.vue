        ...
        <div id="benchmarkingTable">
          ...
          <div class="table-container">
            <table class="table table-fixed table-bordered cuartiles-table" v-if="quartileData.length > 0">
              <thead>
                <tr>
                  <th class="agentHeader">Participants</th>
              ...
              <tbody>
                <tr v-for="(item, index) in quartileData" :key="item.agent_id"
...
onMounted(async () => {
  ...
  agentID.value = data.value.challenge_participants.map((participant) => participant.agent_id);
  allAgentID.value = data.value.challenge_participants.map((participant) => participant.agent_id);
...
const updatePlotOnSelection = (traceIndex) => {
  ...
  // (Add hidden) Hide or show the agent based on its current state
  const agentHidden = dataPoints.value[traceIndex].hidden;

  // If hiding the trace, check if there are at least 4 visible traces
  if (!agentHidden) {
    const visibleAgents = dataPoints.value.filter((agent) => !agent.hidden);
  ...
  dataPoints.value[traceIndex].hidden = !agentHidden;

  // Filter visible agents
  const updatedVisibleAgents = dataPoints.value.filter((agent) => !agent.hidden);

  // Calculate the new Pareto Frontier with the visible agents
  ...
  if (viewKmeans.value === true) {
      ...
      const visibleAgents = agentID.value.map((agent, index) => ({
        name: agent,
      ...
      createShapeClustering(updatedVisibleAgents, visibleAgentNames)
  ...
  if (viewSquare.value === true) {
    ...
    const updatedXCoordinates = ref(updatedVisibleAgents.map((participant) => participant[0]))
    const updatedYCoordinates = ref(updatedVisibleAgents.map((participant) => participant[1]))
    ...
    calculateQuartiles(updatedXCoordinates.value, updatedYCoordinates.value, visibleAgentNames);
    optimalView()
  }
...
const toggleQuartilesVisibility = () => {
  ...
  calculateQuartiles(xValues.value, yValues.value, agentID.value);
  optimalView();
};

// Calculate square quartiles
const calculateQuartiles = (xValues, yValues, agentID) => {
  ...
  sortAgentsForSquare(better, agentID, cuartilesX, cuartilesY, xValues, yValues)
...
const sortAgentsForSquare = (better, visibleAgentID, cuartilesX, cuartilesY, xValues, yValues) => {
  quartileData.value = [];
  allAgentID.value.forEach((agent) => { // Iterate over all agents
    ...
    quartileData.value.push({ agent_id: agent, cuartil: cuartil, label: label });
  });
}
...
const getDiagonalQuartile = (x_values, y_values) =>{
  ...
  // Get Annotations
  let annotationDiagonal = asigneQuartileDiagonal(agents_not_hidden, first_quartile, second_quartile, third_quartile)
  
  // Diagonal Q. Table
  createTableDiagonal(agents_not_hidden)
...
const asigneQuartileDiagonal = (dataAgents, first_quartile, second_quartile, third_quartile) => {
...
// Create Table
const createTableDiagonal = (visibleAgent) => {
  quartileData.value = [];

  allAgentID.value.forEach((agent) => {
    const agentName = agent;
    const visibleAgentInfo = visibleAgent.find(item => item[0] === xValues.value[allAgentID.value.indexOf(agent)]);
    ...
    quartileData.value.push({ agent_id: agentName, cuartil: quartile, label: label });
  });
};
...
const toggleKmeansVisibility = () => {
  ...
  createShapeClustering(dataPoints.value, agentID.value);

};


const createShapeClustering = (dataPoints, agentIDVisible) => {
  clusterMaker.k(4);
  ...
  createDataPointForTables(agentIDVisible, groupedDataPoints)
...
const createDataPointForTables = (visibleAgents, groupedDataPoints) => {
  quartileData.value = [];
  allAgentID.value.forEach((agent) => {
